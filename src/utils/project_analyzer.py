...
import re
import pandas as pd # NEU: Import für pandas
from datetime import datetime, timedelta, date
from collections import Counter

# (Pfad-Konfiguration und Imports bleiben gleich)
...
class ProjectAnalyzer:
    # __init__, _gather_all_activities, analyze_dynamics, analyze_status, _calculate_epic_status_durations und die Zeit-Hilfsmethoden bleiben unverändert
    ...
    def analyze_dynamics(self) -> dict:
        ...
        return {
            "analysis_metadata": {
                ...
                "total_activities_last_4_weeks": total_activities_last_4_weeks,
    ...
    def _calculate_epic_status_durations(self) -> dict:
        ...
        if last_status_name in TARGET_STATUSES and self.all_activities:
    ...
    def create_status_timeline_plot(self, status_changes: list):
        ...
        for change in status_changes:
            ...
            else: child_dates.append(dt_object)
    ...
    # --- NEUE METHODE ZUR GRAFISCHEN DARSTELLUNG VON AKTIVITÄT UND TIME CREEP ---
    def create_activity_and_creep_plot(self, time_creep_events: list):
        """
        Erstellt eine kombinierte Grafik: Säulendiagramm für monatliche Aktivität
        und eine Swimlane für TIME_CREEP-Ereignisse.
        """
        if not self.all_activities:
            logger.warning("Keine Aktivitäten für die Grafikerstellung vorhanden.")
            return

        # 1. Daten für das Säulendiagramm aufbereiten (monatliche Aktivität)
        df = pd.DataFrame(self.all_activities)
        # KORRIGIERT: Konvertiert alle Zeitstempel in die einheitliche UTC-Zeitzone, um den TypeError zu beheben.
        df['timestamp_dt'] = pd.to_datetime(df['zeitstempel_iso'], utc=True)
        # KORRIGIERT: Verwendet 'ME' (Month End) statt des veralteten 'M'.
        monthly_counts = df.set_index('timestamp_dt').resample('ME').size()

        # 2. Daten für die TIME_CREEP Swimlane aufbereiten
        ...
        # 3. Plot erstellen mit zwei Subplots (geteilte X-Achse)
        fig, (ax1, ax2) = plt.subplots(
            2, 1, sharex=True, figsize=(20, 8),
            gridspec_kw={'height_ratios': [1, 4]} # Swimlane oben ist kleiner
        )
        fig.suptitle(f"Projektübersicht für Epic: {self.epic_id}", fontsize=18)

        # 4. Obere Grafik (Swimlane für TIME_CREEP)
        ...
        # 5. Untere Grafik (Säulendiagramm für Aktivitäten)
        ...
        # 6. Gemeinsame X-Achse formatieren
...
if __name__ == '__main__':
    ...
    # NEU: Aufruf der neuen Plot-Funktion
